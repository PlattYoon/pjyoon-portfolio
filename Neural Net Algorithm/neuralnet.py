...
def random_init(shape):
    ...
    NOTE: Please use np.random.uniform(). Other random function might not 
    pass the autograder.
...
class Linear:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        #  the backward pass
        self.x_with_bias = np.concatenate(([1.0],x))
        return self.w @ self.x_with_bias
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        self.dw = np.outer(dz,self.x_with_bias)
        dx_with_bias = self.w.T @ dz
        dx = dx_with_bias[1:]
        return dx
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        y_hat, loss = self.smce.forward(b, y)
        return y_hat, loss
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        gb = self.smce.backward(y, y_hat)
        gz = self.linear2.backward(gb)
        ga = self.sigmoid.backward(gz)
        _ = self.linear1.backward(ga)
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        for i in range(X.shape[0]):
            _, loss = self.forward(X[i], int(y[i]))
            total_loss += loss
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(X_shuf.shape[0]):
                y_hat, _ = self.forward(X_shuf[i], int(y_shuf[i]))
                self.backward(int(y_shuf[i]), y_hat)
                self.step()
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        labels = []
        for i in range(X.shape[0]):
            y_hat, _ = self.forward(X[i], int(y[i]))
