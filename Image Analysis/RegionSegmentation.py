...
def get_region_segmentation(image, show=False):
    """
    ...
    if show:
        ...
        ax[0].set_title("grayscale"); ax[0].set_axis_off()
        ax[1].imshow(elevation, cmap="magma")
        ax[1].set_title("elevation (sobel)")
        ax[1].set_axis_off()
        ax[2].imshow(mask, cmap="gray")
        ax[2].set_title(f"watershed mask: {fname}")
        ax[2].set_axis_off()
        plt.tight_layout()
        plt.show()
