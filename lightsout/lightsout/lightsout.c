...
board_t make_move(board_t B, index_t row, index_t col) {
    ...
    new_board = bitvector_flip(new_board, pressed_idx);
    
    if (row > 0) {
        index_t up_idx = (row - 1) * B.width + col;
        new_board = bitvector_flip(new_board, up_idx);
    }
    
    if (row < B.height - 1) {
        index_t down_idx = (row + 1) * B.width + col;
        new_board = bitvector_flip(new_board, down_idx);
    }
    
    if (col > 0) {
        index_t left_idx = row * B.width + (col - 1);
        new_board = bitvector_flip(new_board, left_idx);
    }
    
    if (col < B.width - 1) {
        index_t right_idx = row * B.width + (col + 1);
        new_board = bitvector_flip(new_board, right_idx);
    }
